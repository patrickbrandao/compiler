...
#----------------------------------------------------------------------------
	...
	_http_get(){
		...
		logit2 -n "[$LABEL] Baixando" "$_hg_url -> "
...
#----------------------------------------------------------------------------
	...
	logit2 "[$LABEL] MD5 do arquivo remoto...:" "$REMOTEMD5FILE_MD5"
	if [ "x$MD5VALUE" = "x" ]; then
		logit2 "[$LABEL] MD5 do arquivo local....:" "(vazio)"
	else
		logit2 "[$LABEL] MD5 do arquivo local....:" "$MD5VALUE"
		# ambos os valores disponiveis, comparar
		if [ "$REMOTEMD5FILE_MD5" = "$MD5VALUE" ]; then
			# Iguais
			logit2 "[$LABEL]" "Download desnecessario, assinaturas MD5 iguais"
			_quit
		else
			# diferentes
			logit2 -n "[$LABEL] Download necessario,"; echoc -c yellow "assinaturas MD5 DIFERENTES"
			NEEDUPDATE=2
		fi
	fi
	...
	if [ "$NEEDUPDATE" = "0" ]; then
		...
		logit2 -n "[$LABEL] Download necessario,"; echoc -c yellow "arquivo local ausente"
	fi
...
#----------------------------------------------------------------------------
	...
	logit2 "[$LABEL] MD5 do arquivo baixado..:" "$LOCALTMPFILE_MD5"
	logit2 "[$LABEL] MD5 da assinatura obtida:" "$REMOTEMD5FILE_MD5"
